...
class OrderApp extends BackendApp
{
	...
	//$city=$row_member['city'];
        ...
        $orders = $model_order->find(array(
		 ...
		 $money_sum=$model_order->getRow("SELECT SUM(order_jifen) orjf,SUM(order_amount) oram,COUNT(*) cou  FROM " . DB_PREFIX . "order WHERE 1=1  ".$conditions." ");
		...
		if($time==5)
		{
			$a_week_ago = gmtime() - 1 * 24 * 3600;
		}
		else
		{
			$a_week_ago = gmtime() - $time * 7 * 24 * 3600;
		}
		$condit .= ' and finished_time > '.$a_week_ago;
		}
		$suozhan=$_GET['suozhan'];
		if(!empty($suozhan))
		{
			$condit .=' and city= '.$suozhan;
			$this->assign('suozhan',$suozhan);
		}
		if(!empty($ordermoney) || !empty($time) || !empty($suozhan))
		{
			if($ordermoney==1)//订单积分
			{
				$order_jz=$model_order->getRow("SELECT SUM(order_jifen) orderjifen FROM " . DB_PREFIX . "order WHERE ".$condit." and status = '" . ORDER_FINISHED . "' ");
				$this->assign('order_jz',$order_jz);
			}
			if($ordermoney==2)//订单总数
			{
				$order_cou=$model_order->getRow("SELECT COUNT(*) cou FROM " . DB_PREFIX . "order WHERE ".$condit."  AND status = '" . ORDER_FINISHED . "'");
				$this->assign('order_cou',$order_cou);
			}
			if($ordermoney==3)//订单总额
			{
				$order_am=$model_order->getRow("SELECT SUM(order_amount) orderamount FROM " . DB_PREFIX . "order WHERE ".$condit." and status = '" . ORDER_FINISHED . "' ");
				$this->assign('order_am',$order_am);
			}
		}
