...
$priv_row=$this->_admin_mod->getrow("select * from ".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0");
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			$recommend=$this->_recommend_mod->getrow("select * from ".DB_PREFIX."recommend where id = '$re_id'");
            if (!$recommend)
            {
...
$row_member=$this->_user_mod->getrow("select * from ".DB_PREFIX."member where user_name = '$user'");
	...
	$city=$priv_row['city'];
		...
		$goods_list = $goods_mod->find(array(
            ...
            'fields' => 'g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, recommended_goods.sort_order, g.closed, g.if_show, views,recommended_goods.recom_id,s.cityid',
            'conditions' => "recommended_goods.recom_id = '$id' and recommended_goods.rcity like '%$city,%' ",
            'limit' => $page['limit'],
            'order' => 'recommended_goods.sort_order',
            'count' => true,
        ));
