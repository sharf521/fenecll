...
//		unset($menu['dashboard']);
    ...
    function _get_news_in_a_week()
    {
        ...
        $user_mod =& m('member');
		$userid=$this->visitor->get('user_id');
	$priv_row=$user_mod->getRow("select * from ".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0 limit 1");
	$this->assign('priv_row',$priv_row);
	...
	$city=$priv_row['city'];
		...
		{
			return array(
				'new_user_qty'  => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "member WHERE reg_time > '$a_week_ago' and city='$city'"),
				'new_store_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE add_time > '$a_week_ago' AND state = 1 and cityid='$city'"),
				'new_apply_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE add_time > '$a_week_ago' AND state = 0 and cityid='$city'"),
				'new_goods_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "goods WHERE add_time > '$a_week_ago' AND if_show = 1 AND closed = 0 and cityhao='$city'"),
				'new_order_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "order WHERE finished_time > '$a_week_ago' AND status = '" . ORDER_FINISHED . "' and city='$city'"),
			);
	...
	if($privs=='all')
	{
        return array(
			...
			'store_yuming' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE state = 1 and domain!=''"),
        );
	}
	else
	{
        return array(
            'user_qty'  => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "member where  city='$city'"),
            'store_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE state = 1 and cityid='$city'"),
            'apply_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE state = 0 and cityid='$city'"),
            'goods_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "goods WHERE if_show = 1 AND closed = 0 and cityhao='$city'"),
            'order_qty' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "order WHERE status = '" . ORDER_FINISHED . "' and city='$city'"),
            'order_amount' => $user_mod->getOne("SELECT SUM(order_amount) FROM " . DB_PREFIX . "order WHERE status = '" . ORDER_FINISHED . "' and city='$city'"),
			'store_yuming' => $user_mod->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "store WHERE state = 1 and domain!='' and cityid='$city'"),
        );
	}
    }
	...
	{
		$this->sessions_mod=& m('sessions');
		
		$riqi=date('Y-m-d H:i:s',strtotime("-1 weeks"));
		$dat=time($riqi);
		$sql="delete from ".DB_PREFIX."sessions where expiry<='$dat'";
		$this->sessions_mod->db->query($sql);
		$this->sessions_mod->db->query("OPTIMIZE TABLE ".DB_PREFIX."sessions ");
		$this->show_message('ok');
	}
