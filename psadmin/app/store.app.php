...
$kaiguan=$this->userpriv_mod->getRow("select mxstore from ".DB_PREFIX."kaiguan ");
...
/*$index=$this->_store_mod->find(array(
    ...
    function edit()
    {
		...
		$result=$this->message_mod->getrow("select user_name from ".DB_PREFIX."member where user_id='$id'");
		$username=$result['user_name'];
		
		
		$userid=$this->visitor->get('user_id');
		$priv_row=$this->message_mod->getrow("select * from ".DB_PREFIX."user_priv where user_id = '$userid' and store_id=0");
		$this->assign('priv_row', $priv_row);
