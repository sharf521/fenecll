...
if($jia_or_jian=="jie")
    ...
    'dq_jifen_dj'=>$dongjiejifen,
	   ...
	   $this->my_money_mod->edit('user_id='.$user_ids,$money_array);
		...
		if($jia_or_jian=="jian")
		{
	   	$this->show_message('user_money_add_jianshaoyonghujinechenggong','返回列表','index.php?module=my_money&act=user_money_list');
	        return;
		}
		if($jia_or_jian=="dong")
		{
	   	$this->show_message('user_money_add_dongjieyonghujinechenggong','返回列表','index.php?module=my_money&act=user_money_list');
	        return;
		}
		if($jia_or_jian=="jie")
		{
	   	$this->show_message('user_money_add_jiedongyonghujinechenggong','返回列表','index.php?module=my_money&act=user_money_list');
	        return;
		}
	   }
	   else
...
$userid=$this->visitor->get('user_id');
	...
	//查询
		...
		$time=$_GET['time'];
		$this->assign('typee',$typee);
		$this->assign('lei',$lei);
		if(!empty($time))
		{
		$this->assign('time',$time);
		$a_week_ago = gmtime() - $time * 7 * 24 * 3600;
		$a_week=date('Y-m-d',$a_week_ago);
		$condit .= ' and time > '.$a_week;
		}
		$suozhan=$_GET['suozhan'];
		if(!empty($suozhan))
		{
			$condit .=' and zcity= '.$suozhan;
			$this->assign('suozhan',$suozhan);
		}
		if(!empty($typee) || !empty($time) || !empty($suozhan))
		{
			if($typee==1)//积分
			{
				$jifen_z=$this->accountlog_mod->getRow("SELECT SUM(jifen) jifenz FROM " . DB_PREFIX . "accountlog WHERE ".$condit."  ");
				$this->assign('jifen_z',$jifen_z);
			}
			if($typee==2)//金额
			{
				$money_z=$this->accountlog_mod->getRow("SELECT SUM(money) moneyz FROM " . DB_PREFIX . "accountlog WHERE ".$condit." ");
...
function cz_yi_shenhe()//caozuo=61
	...
	if(!empty($suoshuzhan))
	{
		...
		$money_sum=$this->my_moneylog_mod->getRow("SELECT SUM(money) mon,SUM(money_feiyong) monf FROM " . DB_PREFIX . "my_moneylog WHERE ".$conditions." status !=0 and leixing=30  ");
		$sum_mon=$money_sum['mon']+abs($money_sum['monf']);	
		$this->assign('sum_mon',$sum_mon);
		$city_row=array();
		$result=$this->city_mod->getAll("select * from ".DB_PREFIX."city");
		$this->assign('result',$result);
		foreach ($result as $var )
...
//提现搜索
	...
	if(!empty($soname))//若用户名不为空
	{
		if(!empty($sotime) and empty($endtime))
		{
			$conditions .=" and user_name like '%$soname%' and riqi>='$sotime' ";
		}
		if(empty($sotime) and !empty($endtime))
		{
			$conditions .=" and user_name like '%$soname%' and riqi<='$endtime 24:59:59' ";
		}
		if(!empty($sotime) and !empty($endtime))
		{
			$conditions .=" and user_name like '%$soname%' and riqi>='$sotime' and riqi<='$endtime 24:59:59' ";
		}
		if(empty($sotime) and empty($endtime))
		{
	...
	else
	{
		...
		if(!empty($sotime) and !empty($endtime))
		{
			$conditions .=" and riqi>='$sotime' and riqi<='$endtime 24:59:59' ";
		}
	...
	if($state==2)//未打款
	{
		$conditions .=" and status1=2 and (status is null or status=2)  ";
	}
	if(!empty($suoshuzhan))
	{
		$conditions .=" and city='$suoshuzhan' ";
	...
	$city=$priv_row['city'];	
		...
		{
		$index=$this->my_moneylog_mod->find(array(
	        'conditions' =>$conditions. " and leixing=40",
            'limit' => $page['limit'],
			'order' => "id desc",
			'count' => true));
		...
		{
			$index=$this->my_moneylog_mod->find(array(
	        'conditions' =>$conditions. " and leixing=40 and city='$city'",
            'limit' => $page['limit'],
			'order' => "id desc",
			'count' => true));
		}
	   
	   $money_sum=$this->my_moneylog_mod->getRow("SELECT SUM(money) mon,SUM(money_feiyong) monf FROM " . DB_PREFIX . "my_moneylog WHERE ".$conditions." and leixing=40  ");
		$sum_mon=abs($money_sum['mon'])+abs($money_sum['monf']);	
		$this->assign('sum_mon',$sum_mon);
...
$user_row=$this->my_money_mod->getRow("select * from ".DB_PREFIX."my_money where user_id='$user_id' limit 1");	
	...
	if($_POST)
	{
		$money_row=$this->my_money_mod->getrow("select * from ".DB_PREFIX."my_moneylog where id='$log_id'");
