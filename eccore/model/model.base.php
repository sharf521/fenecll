...
class BaseModel extends Object
{
	...
	{
		//消费者的推荐人操作
		$row_user=$this->db->getRow("select yaoqing_id,city,user_name from ".DB_PREFIX."member where user_id='$user_id' limit 1");
		$us_username=$row_user['yaoqing_id'];//消费者的推荐人的用户名
		$xiaofei_city=$row_user['city'];
		$xiaofei_username=$row['user_name'];
	...
	if(!empty($us_username))//推荐人不为空
	{
		...
		$this->db->query("insert into ".DB_PREFIX."accountlog (jifen,time,user_id,user_name,zcity,type,s_and_z,dq_money,dq_jifen,beizhu)  values ('-$jifen_zong','$riqi','$user_id','$xiaofei_username','$xiaofei_city',38,2,'$zong_money','$new_zong_jifen','$beizhu')");
