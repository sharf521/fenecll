...
//$url=$_SERVER['HTTP_HOST'];//获得当前网址
	...
	$this->assign('cityid',$city_id);	
	$recom20=$this->_city_mod->getrow("select recom_name from ".DB_PREFIX."recommend where recity = '$city_id' and recom_id=20");
	$recom21=$this->_city_mod->getrow("select recom_name from ".DB_PREFIX."recommend where recity = '$city_id' and recom_id=21");
	$recom22=$this->_city_mod->getrow("select recom_name from ".DB_PREFIX."recommend where recity = '$city_id' and recom_id=22");
	$recom23=$this->_city_mod->getrow("select recom_name from ".DB_PREFIX."recommend where recity = '$city_id' and recom_id=23");
	$recom24=$this->_city_mod->getrow("select recom_name from ".DB_PREFIX."recommend where recity = '$city_id' and recom_id=24");	
...
$this->assign('recom20', $recom20);
	   ...
	   ");
		$img_goods_list1 = $recom_mod->get_recommended_goods(23, 8, true);
	   ...
	   ");
		$img_goods_list2 = $recom_mod->get_recommended_goods(20, 6, true);
        ...
        {	
			...
			if($key1==1 || $key1==0)
			{
			$img_goods1[$key1]=$img_goods_list2[$key1];
			$this->assign('img_goods1', $img_goods1); 
			}
			if($key1==2 || $key1==3)
			{
			$img_goods2[$key1]=$img_goods_list2[$key1]; 
			$this->assign('img_goods2', $img_goods2);  
			}
			if($key1==4 || $key1==5)
			{
			$img_goods3[$key1]=$img_goods_list2[$key1]; 
	   ...
	   ");
		$img_goods_list3 = $recom_mod->get_recommended_goods(24, 8, true);
	   ...
	   $this->assign('adv_list5', $adv_list5);
		$img_goods_list4 = $recom_mod->get_recommended_goods(21, 6, true);
		...
		foreach ($img_goods_list4 as $key1 => $good)
        {
			...
			if($key1==1 || $key1==0)
			{
			$img_goods11[$key1]=$img_goods_list4[$key1];
			$this->assign('img_goods11', $img_goods11); 
			}
			if($key1==2 || $key1==3)
			{
			$img_goods22[$key1]=$img_goods_list4[$key1]; 
			$this->assign('img_goods22', $img_goods22);  
			}
			if($key1==4 || $key1==5)
			{
			$img_goods33[$key1]=$img_goods_list4[$key1]; 
			$this->assign('img_goods33', $img_goods33);  
			}
		}
	   ...
	   ");
		...
		$img_goods_list5 = $recom_mod->get_recommended_goods(22, 8, true);
	...
	*/
	...
	$time=date('Y-m-d H:i:s');
	 $this->adv_mod =& m('adv');
	   ...
	   st.cityid='$city_id' and st.dengji=1 and st.state=1 order by st.add_time desc limit 4
	   ");
	   
	   /*if(empty($star))
	   {
	   	$sql="SELECT st.*,m.user_name, case st.cityid 
					when '$city_id' then 10
					else 1
					end as c " .
                    "FROM " . DB_PREFIX . "store AS st " .
                    "   LEFT JOIN " . DB_PREFIX . "member AS m ON st.store_id = m.user_id " .
                    " WHERE st.dengji=1 and st.state=1" . 
                    " ORDER BY c desc " .
					" LIMIT 4";
		$star=$this->adv_mod->getAll($sql);			
	   }*/
	   ...
	   //公告
			...
			{
				$cateid=$acategory_mod->get_ACC(ACC_NOTICE);
			}
			
            $news = $article_mod->find(array(
                'conditions'    => 'cate_id=' . $cateid . ' AND if_show = 1 and (city=1 or city ='.$city_id .')',
			...
			foreach ($news as $key=>$val)
			{
				...
				{
					$i++;
					$news[$key]['title']=substr($val['title'],0,24);
					$news[$key]['diyi']=1;
				}
			}
	   ...
	   $order_goods_mod =& m('ordergoods');
			...
			//首页收缩广告
		$adv_list17=$this->adv_mod->getrow(
	   "select * from ".DB_PREFIX."adv where 
	   adv_city='$city_id' and type='17' and start_time<='$time' and end_time>='$time' order by riqi desc limit 1
	   ");
	   $this->assign('adv_list17', $adv_list17);
		
		//对联广告
		$adv_list16=$this->adv_mod->getrow(
	   "select * from ".DB_PREFIX."adv where 
	   adv_city='$city_id' and type='16' and start_time<='$time' and end_time>='$time' order by riqi desc limit 1
	   ");
	   $this->assign('adv_list16', $adv_list16);
