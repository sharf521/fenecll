...
function gonghuo()
{
	...
	$row_member=$this->member_mod->getrow("select status,city from ".DB_PREFIX."gonghuo_xinxi where user_id = '$user_id'");
	...
	if($_POST)
	{
		 ...
		 $goo=$this->member_mod->getrow("select default_image from ".DB_PREFIX."goods where goods_id = '$goodsid'"); 
		 ...
		 $lingshou_price = (float)($_POST['lingshou_price']);
		// $lingshou_jifen = trim($_POST['lingshou_jifen']);
	 	...
	 	$beizhu=$htmlData;
	  	...
	  	if($beizhu=="")
	  	{
		   $this->show_warning('shangpinbuneng');
		   return;
	  	}
	 
	 	$canshu=$this->member_mod->can();

	    $formatted = sprintf("%.2f", $lingshou_price);
	    $jifen_price=format_price($formatted*$canshu['jifenxianjin']);
	...
	else
	{
	   ...
	   {
	   	  $mem=$this->member_mod->getrow("select * from ".DB_PREFIX."gonghuo_xinxi where user_id = '$user_id'");
...
function caigouxinxi()
{
	...
	foreach($xinxi as $key=>$var)
	{
		$caiid=$var['cai_id'];
		$row=$this->caigou_mod->getRow("select level from ".DB_PREFIX."member where user_id='$caiid' limit 1");
		$bb=explode(',',$row['level']);
		if(in_array(1,$bb))
		{
			$xinxi[$key]['leve']=1;
		}
...
function ghshenhe()
{
     ...
     $caigou_row=$this->caigou_mod->getrow("select * from ".DB_PREFIX."caigou where id='$id'");
	 $cai_id=$caigou_row['cai_id'];
	 $cai_name=$caigou_row['cai_name'];
	 ...
	 $gonghuo_row=$this->caigou_mod->getrow("select yu_kucun,zong_kucun from ".DB_PREFIX."gonghuo where gh_id='$gh_id'");
	 $zong_kucun=$gonghuo_row['zong_kucun'];
	 $yu_kucun=$gonghuo_row['yu_kucun'];
	 $money_row=$this->caigou_mod->getrow("select * from ".DB_PREFIX."my_money where user_id='$cai_id'");
	 $member_row=$this->caigou_mod->getrow("select * from ".DB_PREFIX."member where user_id='$cai_id'");
	 ...
	 if($_POST)
	 {
		if($status==1)
	    {
			...
			//更新总账户
				  ...
				  $this->member_mod =& m('member');
				  $user_row=$this->member_mod->getrow("select * from ".DB_PREFIX."member where user_id='$cai_id'");
				  $userrow=$this->member_mod->getrow("select * from ".DB_PREFIX."member where user_id='$gong_id'");		
...
function fabu()
{
	...
	$cai=$this->member_mod->getRow("select num from ".DB_PREFIX."caigou where id='$fabuid' limit 1");
	$num=$cai['num'];
	...
	if(empty($goodid))
	{
		$goods_name=$row_huo['goods_name'];
		$goods_brand=$row_huo['goods_brand'];
		$chanpin=$row_huo['chanpin'];
		$lingshou_price=$row_huo['lingshou_price'];
		$descr=addslashes($row_huo['beizhu']);
		$jifenxianjin=$canshu['jifenxianjin'];
		$lv31=$canshu['lv31'];
		$lv21=$canshu['lv21'];
		$jifen_price=$lingshou_price*$jifenxianjin*(1+$lv31);
		$vip_price=$lingshou_price*$jifenxianjin*(1+$lv21);
	}
	else
	{
	...
	'brand'=>$goods_brand,
	'type'=>$goods_material,
	'default_image'=>$chanpin,
	'price'=>$lingshou_price,
	'jifen_price'=>$jifen_price,
	'vip_price'=>$vip_price,
	'cityhao'=>$mcity,
	'description'=>$descr,
	...
	'shipping_id'=>	$row_huo['shipping_id'],
	'weight'=>	$row_huo['weight'],
	'volume'=>	$row_huo['volume']
	);		
	...
	else
	{
		...
		$file_id=$this->image_mod->add($add_image);
	}
	//编辑goods表
	$edit_good=array(
...
function gh_edit()
{
      ...
      if (!IS_POST)
      {
       ...
       else
       {
			...
			$data['jifen_price']    =   $_POST['jifen_price'];
			$data['weight']    =   $_POST['weight'];
			$data['volume']    =   $_POST['volume'];
			$data['source']    =   $_POST['source'];
			$data['zong_kucun']    =   $_POST['zong_kucun'];
			$data['yu_kucun']    =   $_POST['zong_kucun'];
...
function ghsq()
{
	...
	$user_id = $this->visitor->get('user_id');	 
	$user_name = $this->visitor->get('user_name');	 
	$row_mem=$this->member_mod->getrow("select * from ".DB_PREFIX."member where user_id = '$user_id'");
	$city=$row_mem['city'];
$xinxi=$this->gonghuo_xinxi_mod->getrow("select * from ".DB_PREFIX."gonghuo_xinxi where user_id = '$user_id'");
	if($_POST)
	{
		 $name = trim($_POST['name']);
		 $sex = trim($_POST['sex']);
	...
	else
	{
        ...
        $this->_curitem('gonghuo');
		$user = $this->visitor->get();
		$gonghuo=$this->gonghuo_xinxi_mod->getrow("select * from ".DB_PREFIX."gonghuo_xinxi where gh_id = '$gh_id'");
	    $this->assign('user', $user);
		$this->assign('gonghuo', $gonghuo);
