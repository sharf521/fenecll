...
class MemberApp extends MemberbaseApp
{
    ...
    function index()
    {
        ...
        $my_money = $this->my_money_mod->getAll("select * from " . DB_PREFIX . "my_money where user_id='$user_id' limit 1");
    ...
    function register()
    {
        ...
        if (!IS_POST) {
            ...
            $this->assign('page_title', Lang::get('user_register') . ' - ');
...
//添加qiandao_log日志
    ...
    function email()
    {
        ...
        if (!IS_POST) {
            ...
            $user = $this->user_mod->getRow("select email from " . DB_PREFIX . "member");
            if ($email == $user['email']) {
