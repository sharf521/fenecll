...
class ApplyApp extends MallbaseApp {
	...
	function index() {
		...
		$row_member = $this->member_mod->getrow("select * from " . DB_PREFIX . "member where user_name = '$user_name'");
		...
		switch ($step) {
			case 1 :
				...
				$this->assign('page_title', Lang :: get('title_step1') . ' - ');
			...
			case 2 :
				...
				if (!IS_POST) {
					...
					$this->assign('page_title', Lang :: get('title_step2') . ' - ');
					...
					if ($sgrade['need_confirm']) {
						$this->show_message('apply_ok', 'index', 'index.php?app=member');
					} else {
						...
						$this->_hook('after_opening', array (
							'user_id' => $store_id
						));
						$this->show_message('store_opened', 'index', 'index.php?app=member');
					}
				}
				break;
			default :
