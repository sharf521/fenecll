...
*/	
		...
		foreach ($goods_list as $key => $goods)
        {
			...
			$sp=$goods_mod->getrow("select region_name from ".DB_PREFIX."store where store_id = '$store_id' limit 1");
			$goods_list[$key]['region']=substr($sp['region_name'],5,100);
        }
	...
	$city_id=$cityrow['city_id'];
		...
		if($new==1)
		{
			$orders="s.store_id desc";
		}
		else
		{
			$orders=empty($_GET['order']) || !in_array($_GET['order'], array('credit_value desc')) ? 'sort_order' : $_GET['order'];
		}
        $model_store =& m('store');
	...
	//$city_id=$row_city['city_id'];
	 ...
	 $city_id=$cityrow['city_id'];
        ...
        if ($_GET['state'] == 'on')
        {
            ...
            $conditions .= ' AND gb.state ='. GROUP_ON .' AND gb.end_time>' . gmtime().' and gb.start_time<=' .gmtime();
        }
        ...
        $this->assign('page_title', Lang::get('groupbuy') .' - ');
	...
	$this->assign('kaiguan',$kaiguan);
	 $this->_curlocal(array(array('text' => Lang::get('youhui'))));
        $this->assign('page_title', Lang::get('youhui').' - ');
...
function guanggaowei($_num,$type)
    ...
    function _get_goods_curlocal($cate_id)
    {
		...
		foreach($curlocal as $key=>$var)
		{
			$title=$var['text'];
		}
		$this->assign('page_title', $title.' - '.Lang::get('shangpin').' - ');
        return $curlocal;
    }
...
$rec=$this->city_mod->getrow("select * from ".DB_PREFIX."recommend where recity = '$cityid' and recom_id=$recom_id");
    ...
    function _get_store_curlocal($cate_id)
    {
        ...
        $curlocal = array(
            array('text' => LANG::get('diangpuliebiao'), 'url' => url('app=category&act=store')),
        );
        foreach ($parents as $category)
        {
		...
		foreach($curlocal as $key=>$var)
		{
		 ...
		 $this->assign('page_title', $title.' - '.Lang::get('dianpu').' - ');
        return $curlocal;
    }
...
function gonghuo()
 {
	   ...
	   $sto=$this->_city_mod->getRow("select store_id from ".DB_PREFIX."store where store_id = '$userid' and state=1 limit 1");
	  if($sto['store_id']=='')
	  {
	  $this->show_message('meiyoushenqing','back_list','index.php?app=member');
	  return;
	  }
    ...
    $this->assign('build_editor', $this->_build_editor(array('name' => 'beizhu')));
        $this->gonghuo_mod =& m('gonghuo');
		  ...
		  $this->assign('page_title', Lang::get('caigou').' - ');
		 $page = $this->_get_page(8); 
       $deng=Lang::get('shenhetongguo');
	   $conditions.=" (suoshu_city='all' or suoshu_city like '%$city_id,%') and ";
...
*/
	...
	function news()
    {
		...
		$this->assign('page_title', Lang::get('dianzhuchongdian') . ' - ');
	$time=date('Y-m-d H:i:s');
...
$adv_list11=$this->_city_mod->getAll("select * from ".DB_PREFIX."adv where 
	...
	function lastest()
	{
		...
		$city_id=$cityrow['city_id'];
		$goods_mod  =& m('goods');
		$page = $this->_get_page(N_PER_PAGE);
		...
		$sql="SELECT g.goods_id,g.goods_name,g.daishou,g.default_image,s.store_name,g.price,g.jifen_price,g.vip_price,gss.comments,gss.sales,s.region_name " .
                    "FROM " . DB_PREFIX . "goods AS g " .
                    "   LEFT JOIN " . DB_PREFIX . "store AS s ON s.store_id = g.store_id " .
					"   LEFT JOIN " . DB_PREFIX . "goods_statistics AS gss ON gss.goods_id = g.goods_id " .
                    " WHERE " . $conditions . 
		...
		$sql1="SELECT COUNT(g.goods_id) cou " .
                    "FROM " . DB_PREFIX . "goods AS g " .
                    "   LEFT JOIN " . DB_PREFIX . "store AS s ON s.store_id = g.store_id " .
                    " WHERE " . $conditions ;
		...
		$good=$goods_mod->getRow($sql1);
		$page['item_count'] = $good['cou'];
		...
		$this->_curlocal(LANG::get('quanbufeilei'), 'index.php?app=category',
                         LANG::get('zuixin'));
        $this->assign('display_mode', $display_mode);
        $this->assign('page_info', $page);	
		$this->assign('goods_list',$goods_list);
		$this->assign('fenlei',$fenlei);	
		$this->display('lastest.goods.html');		
	}
	...
	function tuijian()
	{
		...
		$conditions=" g.if_show = 1 AND g.closed = 0 AND s.state = 1 ";
		$sql="SELECT  g.goods_id, g.goods_name, g.default_image, gs.price, gs.stock,gs.vip_price,gs.jifen_price,g.daishou,g.subhead,gss.comments,gss.sales,s.region_name " .
                    " FROM ". DB_PREFIX ."goods AS g " .
                    "   LEFT JOIN ".DB_PREFIX."goods_spec AS gs ON g.default_spec = gs.spec_id " .
                    "   LEFT JOIN ".DB_PREFIX."store AS s ON g.store_id = s.store_id " .
					"   LEFT JOIN ".DB_PREFIX."goods_statistics AS gss ON gss.goods_id = g.goods_id " .
                    " WHERE " . $conditions . 
                    " AND g.cate_id_1 IN( " . $cate_id .  ")" .
	   ...
	   $this->_curlocal(LANG::get('quanbufeilei'), 'index.php?app=category',
		...
		$this->assign('goods_list',$goods_list);
		$this->assign('fenlei',$fenlei);	
		$this->display('zhuanti.goods.html');		
...
/* 其他检索条件 */
		...
		$regions = $model_store->list_regions();
		$stores=$model_store->getAll("select s.store_name,s.owner_name,s.store_logo,s.store_id,s.credit_value,s.region_name from ".DB_PREFIX."store s left join ".DB_PREFIX."category_store c on s.store_id=c.store_id left join ".DB_PREFIX."scategory sc on sc.cate_id=c.cate_id  where sc.cate_id=$cate_id and s.state=1 ".$conditions." limit {$page['limit']}");
		
		$st=$model_store->getAll("select s.store_id from ".DB_PREFIX."store s left join ".DB_PREFIX."category_store c on s.store_id=c.store_id left join ".DB_PREFIX."scategory sc on sc.cate_id=c.cate_id  where sc.cate_id=$cate_id and s.state=1 ".$conditions);
        ...
        $page['item_count']=count($st); 
		
        $this->_format_page($page);
		$lei=$_GET['lei'];
        /* 当前位置 */
	   if($lei==1)
       $this->_curlocal(array(array('text' => Lang::get('fuzh'))));
	   if($lei==2)
       $this->_curlocal(array(array('text' => Lang::get('huazh'))));
	   if($lei==3)
       $this->_curlocal(array(array('text' => Lang::get('phone'))));
        $scategorys = $this->_list_scategory();
        $this->assign('stores', $stores);
        $this->assign('regions', $regions);
        $this->assign('cate_id', $cate_id);
	...
	{
		$riqi=date('Y-m-d');
		$dir = "E:/WWW/fenecll/ceshi/$riqi";	// 路径 + 用户名
		$items = array_diff(scandir($dir), array('.', '..'));		// 去掉 . 和 ..
		$fen_files = array();// 初始 fen 文件数组
		$ucenter_files = array();// 初始 ucenter 文件数组
		$pay_files = array();// 初始 pay 文件数组
		foreach($items as $file)
	    {		// 循环每一个文件
			//if(strtolower(pathinfo($file, PATHINFO_EXTENSION)) == 'txt')
			//{	// 如果文件以 txt 结尾	
				//$xml_files[filemtime("$dir/$file")] = "$dir/$file";		// 保存为 array(文件最后修改时间 => 文件路径);
			//}	
			
			$filename=strtolower(pathinfo($file, PATHINFO_BASENAME)); //文件名字
			$filena=explode('_',$filename);
			$filename_head=$filena[0]; 
			if($filename_head=="fenecll")
			{
				$fen_files[filemtime("$dir/$file")] = "$dir/$file";
			}
			
			if($filename_head=="ucenter")
			{
				$ucenter_files[filemtime("$dir/$file")] = "$dir/$file";
			}
		...
		asort($fen_files);	// 函数用于对数组单元从低到高进行排序并保持索引关系
		asort($ucenter_files);
		//arsort($fen_files);
		print_r($fen_files);
		$i=0;
		$j=0;
		foreach($fen_files as $key=>$var)
		{
			$fen[$i]['riqi']=$key;
			$fen[$i]['txt']=$var;
			$i++;
		}
		foreach($ucenter_files as $key=>$var)
		{
			$uc[$j]['riqi']=$key;
			$uc[$j]['txt']=$var;
			$j++;
		}
		rsort($fen);//对数组单元进行由高到低排序
		rsort($uc);
		echo $fen[0]['txt'];
	...
	function ftp()
	{
		...
		$riqi=date('Y-m-d');
		$fen_files = array();// 初始 fen 文件数组
		$alg_files = array();// 初始 fen 文件数组
		$algnew_files = array();// 初始 fen 文件数组
		if($can=='fenecll' || $can=='pay' || $can=='ucenter')
		{
			$dir = "F:/backup/mysql/$riqi";	// 路径 + 用户名
			$items = array_diff(scandir($dir), array('.', '..'));		// 去掉 . 和 ..
			foreach($items as $file)
	    	{		// 循环每一个文件
				$filename=strtolower(pathinfo($file, PATHINFO_BASENAME)); //文件名字
			...
			rsort($fen);
			$name=pathinfo($fen[0]['txt'], PATHINFO_BASENAME);
			$new_fen="beifen/".$name;
			$ftp = new class_ftp('116.255.133.71',802,'backup','backup'); // 打开FTP连接
			$ftp->up_file($fen[0]['txt'],$new_fen); // 上传文件
		}
		if($can=='new')
		{
			$dir1 = "F:/backup/mssql/$riqi";	// 路径 + 用户名
			$items1 = array_diff(scandir($dir1), array('.', '..'));
			foreach($items1 as $file)
	   		{		
				$filename=pathinfo($file, PATHINFO_BASENAME); 
				$filena=explode('_',$filename);
				$filename_head=$filena[0]; 
				$filename_middle=$filena[1];
				if($filename_head=="Algrithm" && $filename_middle!=$can)
				{
					$alg_files[filemtime("$dir1/$file")] = "$dir1/$file";
				}
				
				if($filename_head=="Algrithm" && $filename_middle==$can)
				{
					$algnew_files[filemtime("$dir1/$file")] = "$dir1/$file";                               
				}
			}
			asort($alg_files);
			asort($algnew_files);
			...
			foreach($alg_files as $key=>$var)
			{
				$alg[$m]['riqi']=$key;
				$alg[$m]['txt']=$var;
				$m++;
			}
			foreach($algnew_files as $key=>$var)
			{
				$algnew[$n]['riqi']=$key;
				$algnew[$n]['txt']=$var;
				$n++;
			...
			rsort($alg);
			rsort($algnew);
			$name1=pathinfo($alg[0]['txt'], PATHINFO_BASENAME);
			$name2=pathinfo($algnew[0]['txt'], PATHINFO_BASENAME);
			$new_alg="beifen/".$name1;
			$new_algnew="beifen/".$name2;
		
			$ftp = new class_ftp('116.255.133.71',802,'backup','backup'); // 打开FTP连接
			$ftp->up_file($alg[0]['txt'],$new_alg); // 上传文件
			$ftp->up_file($algnew[0]['txt'],$new_algnew); // 上传文件	
		}
