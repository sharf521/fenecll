...
class My_storeApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        if (!IS_POST) {
            ...
            if (!$this->_store_mod->unique($store_name, $this->_store_id)) {
                $this->show_warning('yicunzai');
                return;
            }
    ...
    function caigou()
    {
        ...
        */
        ...
        $member_row = $this->member_mod->getrow("select * from " . DB_PREFIX . "member where user_id='$user_id'");
        $mcity = $member_row['city'];
        $lev = $member_row['level'];
        $riqi = date('Y-m-d H:i:s');
        $gonghuo_row = $this->member_mod->getrow("select * from " . DB_PREFIX . "gonghuo where gh_id='$gh_id'");
        $zong_kucun = $gonghuo_row['zong_kucun'];
        $yu_kucun = $gonghuo_row['yu_kucun'];
        $gong_id = $gonghuo_row['user_id'];
        $gong_name = $gonghuo_row['user_name'];
        $chanpin = $gonghuo_row['chanpin'];
        $status = $gonghuo_row['status'];
        $lingshou_price = $gonghuo_row['lingshou_price'];
        $jifen_price = $gonghuo_row['jifen_price'];
        ...
        if ($_POST) {
            ...
            if ($status != 1) {
                $this->show_warning('cishangpin');
            }
            if ($num <= 0) {
                $this->show_warning('caigoubuneng');
                return;
            }
            ...
            $huo = $this->gonghuo_mod->getrow("select * from " . DB_PREFIX . "gonghuo where gh_id = '$gonghuo_id'");
            $this->assign('kaiguan', $kaiguan);
            $this->assign('huo', $huo);
    ...
    function gh_xiangqing()
    {
        ...
        $shippings = $this->gonghuo_mod->getRow("select shipping_name,cod_regions from " . DB_PREFIX . "shippings where shipping_id = '$shipping_id' limit 1");
        $cod_regions = unserialize($shippings['cod_regions']);
        $gonghuo['shipping_name'] = $shippings['shipping_name'];
        if ($cod_regions) {
            foreach ($cod_regions as $key1 => $val) {
                if ($key1 == 0) {
                    $gonghuo['one_price'] = $val['price'];
                }
            }
        }
        ...
        $xinxi = $this->gonghuo_mod->getrow("select * from " . DB_PREFIX . "gonghuo_xinxi where user_id = '$user_id'");
        if ($gonghuo['ziliao']) {
