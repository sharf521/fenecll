...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        foreach ($goods_list as $key => $goods)
        {
			...
			$gg=$this->_goods_mod->getrow("select status from ".DB_PREFIX."gonghuo where user_id = '$user_id' and goods_id='$goodid' limit 1");
			$goods_list[$key]['stat']=$gg['status'];
			$shippingid=$goods['shipping_id'];
			if($goods['daishou']==1)
			{
				$gongid=$goods['gong_id'];
				$ship=$this->_goods_mod->getRow("select cod_regions from ".DB_PREFIX."shippings sh left join ".DB_PREFIX."gonghuo gh on gh.shipping_id=sh.shipping_id where gh.gh_id='$gongid' limit 1");
			}
			else
			{
		...
		$g=$this->member_mod->getrow("select status from ".DB_PREFIX."gonghuo_xinxi where user_id = '$user_id'"); 
		$this->assign('g',$g);
	...
	$row_member=$this->member_mod->getRow("select * from ".DB_PREFIX."member where user_id = '$user' limit 1");
    ...
    {
	$user=$this->visitor->get('user_id');
	$row_member=$this->member_mod->getrow("select * from ".DB_PREFIX."member where user_id = '$user'");
	$city=$row_member['city'];
...
$shipping=$this->_uploadedfile_mod->getAll("select * from ".DB_PREFIX."shippings where store_id = " .$this->_store_id ." ");
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			//$uploadedfiles=$this->_uploadedfile_mod->db->getAll("select goods_id,image_url,thumbnail from ".DB_PREFIX."goods_image where goods_id=$id order by image_id");
	...
	$spec=$huo['spec_id'];*/
            ...
            $this->show_message('edit_ok',
                'back_list', 'index.php?app=my_goods&page= '. $pag,
   ...
   function spec_edit()
   {
        ...
        else
        {
            ...
            foreach ($data as $key => $val)
            {
                ...
                {
                    $default_spec = array('price' => $val['price'],'jifen_price' => $val['jifen_price'],'vip_price' => $val['vip_price']);
                }
   ...
   function ajax_col()
   {
       ...
       elseif (in_array($column, array('price','jifen_price','vip_price', 'stock', 'sku')))
       {
           ...
           foreach ($records as $record)
            {
				...
				foreach($_imgs as $_img)
				{
				 ...
				 $goods = array(
                    ...
                    'spec_qty'            => 0,
                    'goods_name'       => addslashes(strip_tags($record['goods_name'])),
                ...
                foreach ($spec_data as $spec)
                {
                    ...
                    if (empty($default_spec))
                    {
                        $default_spec = array('default_spec' => $spec_id, 'price' => $spec['price'],'jifen_price'=>$spec['jifen_price'],'vip_price'=>$spec['vip_price']);
                    }
                }
	...
	{
		 $user_id=$this->visitor->get('user_id');
		 $ghxx=$this->_goods_mod->getrow("select * from ".DB_PREFIX."gonghuo_xinxi where user_id = '$user_id'");
		 $status=$ghxx['status'];
