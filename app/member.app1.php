...
$qiandao=$this->my_money_mod->getrow("select riqi,times,status from ".DB_PREFIX."qiandao where user_id=$user_id");
	$riqi=date('Y-m-d');
	 $mfbb=$this->member_mod->getrow("select fbb,daxiaozhuo,level,vip from ".DB_PREFIX."member where user_id=$user_id");
	...
	//echo $id;
	 $this->member_mod =& m('member');
	 $us=$this->member_mod->getrow("select user_name from ".DB_PREFIX."member where user_id = '$id'");
	 $this->assign('us', $us);
