...
class GoodsApp extends StorebaseApp
{
	...
	$mem=$this->_goods_mod->getrow("select vip from ".DB_PREFIX."member where user_id='$user_id'");
	...
	$row=$this->_goods_mod->getrow("select store_id,erweima from ".DB_PREFIX."goods where goods_id='$id'");
    ...
    function qa()
    {
        ...
        $goods_qa =& m('goodsqa');
         $id = intval($_GET['id']);
		$row_good=$this->_goods_mod->getrow("select * from ".DB_PREFIX."goods where goods_id = '$id'");
    ...
    function _assign_common_info($data)
    {
		...
		$can=$this->_goods_mod->can();
		$goods['baoyou_jifen']=$goods['baoyou_money']*$can['jifenxianjin'];
		$shippings=$this->_goods_mod->getRow("select * from ".DB_PREFIX."shippings where shipping_id = '$shipping_id' limit 1");
		$goods['shipping_name']=$shippings['shipping_name'];
		$goods['typeid']=$shippings['typeid'];
		$goods['regions']=unserialize($shippings['cod_regions']);
		/*if(is_array($goods['regions']))
		{
			foreach($goods['regions'] as $key=>$var)
			{
				if($key==0)
				{
				$goods['areaname']=$var['areaname'];
				$goods['one']=$var['one'];
				$goods['one_price']=$var['price'];
				}
			}
		}*/
		$goods['description']=str_ireplace('z-index','',$goods['description']);    
    ...
    function _get_sales_log($goods_id, $num_per_page)
    {
        ...
        $sales_list = $order_goods_mod->find(array(
            ...
            'fields'=> 'buyer_id, buyer_name, add_time, anonymous, goods_id, specification, price, quantity, evaluation,jifen',
            'count' => true,
            'order' => 'add_time desc',
            'limit' => $page['limit'],
        ));
