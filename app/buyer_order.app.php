...
/* by:xiaohei QQ:77491010 商付通 更新商付通定单状态 开始******************************************************/
	...
	$sell_jifen=$order_row['fanhuan_jia'];//返还价
	...
	$orderextm=$this->order_mod->getRow("select fee,shipping_fee from ".DB_PREFIX."order_extm where order_id='$order_id' limit 1");//运费
	
	$fee_money=$orderextm['shipping_fee'];//运费钱
	$fee_nojifen=$fee_money*$jifenxianjin;
	$fee_jifen=$orderextm['fee'];//运费积分
	
	$real_taijifen=$order_jifen-$fee_jifen;//除去运费的积分
	$real_money=$order_amount-$fee_money;//除去运费的钱
	$real_jifen=$real_money*$jifenxianjin;
	...
	if($is_gh==1)//供货商
	{
	    $go_row=$this->order_mod->getRow("select user_id,user_name,shengyukucun from ".DB_PREFIX."gonghuo where gh_id='$gh_id' limit 1");
		$shengyukucun=$go_row['shengyukucun'];
...
//买家对接开始
		if(empty($order_row['zhe_jifen']))//是否参与打折
		{
...
//更新数据		
		if($is_gh==1)//更新供货人的金钱或积分
		{
			if(in_array(1,$bb))//若是付费商家
			{
				if($go_gettype==1)
				{
					$gonghuo_jifen=$real_jifen*(1-$daishou_bili);
					$gonghuo_money=0;
				}
				else
				{
					$gonghuo_money=$real_money*(1-$daishou_bili);
					$gonghuo_jifen=0;
				}
				...
				$this->canshu_mod->edit('id=1',$edit_account);
				 ...
				 $notice=str_replace('{2}',$real_jifen,$notice);
				 ...
				 $add_notice=array(
					...
					$this->my_money_mod->edit('user_id='.$sell_user_id,array('yingde_jifen'=>$sell_yingde_jifen+$real_jifen));//添加卖家的累计应得积分
					//$this->order_mod->edit('order_id='.$order_id,array('yingde_jifen'=>$jifen_zhesuan));//添加卖家的应得积分
					$this->order_mod->edit('order_id='.$order_id,array('yingde_jifen'=>$real_jifen));//添加卖家的应得积分
			   $ri=date('Y-m-d H:i:s');
			...
			else//若是免费商家
			{
			   ...
			   $shouyi_jifen_count=0;
				...
				if($go_gettype==1)
				{
					$gong_shouyi=$real_jifen-$shouyi_jifen_count;//供货人的收益积分
					$gong_shouyi_money=0;
				}
				else
				{
					$gong_shouyi_money=$real_money;//供货人的收益钱
			...
			if($sell_gettype==1)
			{
				$new_money_sell=array('duihuanjifen'=>$sell_duihuanjifen+$fee_nojifen);
				$fee_money=0;
			}
			else
			{
				$new_money_sell=array('money'=>$sell_money+$fee_money);
				$fee_jifen=0;
			}
			$new_money = $sell_money+$fee_money;
			$new_duihuanjifen = $sell_duihuanjifen+$fee_nojifen;				
			$sell_money1=array(
			  'jifen'=>$fee_nojifen,
			  'money'=>$fee_money,
			  'time'=>$riqi,
			...
			$addaccount1=array(					
				...
				's_and_z'=>2,
				'beizhu'=>$log_text,
				'dq_money'=>$account_zongmoney-$fee_money,					
				'dq_jifen'=>$account_zongjifen-$fee_nojifen,
				'xiaofei'=>$buyer_user_name,
				'shangjia'=>$sell_username,
				'gonghuoshang'=>$gong_username
			);			
			...
			$edit_account1=array(					
			  'zong_jifen'=>$account_zongjifen-$fee_nojifen,
			  'zong_money'=>$account_zongmoney-$fee_money
			);		
		    $this->my_money_mod->edit('user_id='.$sell_user_id,$new_money_sell);
		    $this->moneylog_mod->add($sell_money1);
			$this->accountlog_mod->add($addaccount1);
			$this->canshu_mod->edit('id=1',$edit_account1);
			$this->order_mod->tuijian($real_jifen,$buyer_user_id,$sell_user_id,$order_sn,$real_taijifen);
	     ...
	     { 		 
			...
			if($sell_gettype==1)
			{
				$new_money_array=array('duihuanjifen'=>$sell_duihuanjifen+$sell_jifen);
				$order_amount=0;
			}
			else
			{
				$new_money_array=array('money'=>$sell_money+$order_amount);
				$sell_jifen=0;
			}		
...
/* by:xiaohei QQ:77491010 商付通 更新商付通定单状态 结束******************************************************/			
			...
			if($is_gh==1)//修改仓库库存
			{
				...
				foreach($order_goods as $goods)
				{
					$quantity=$quantity+$goods['quantity'];
				}
				$shyukucun=$shengyukucun-$quantity;
			$this->gonghuo_mod->edit('gh_id='.$gh_id,array('shengyukucun'=>$shyukucun));
			}
