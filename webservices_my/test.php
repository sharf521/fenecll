...
/*$sql="select user_id,user_name,email,password,real_name,web_id from {member} order by user_id";
$result=$db->get_all($sql);
foreach($result as $row)
{
	echo "insert into dw_user( type_id,username,password,email,ws_user_id,uc_user_id,areaid) values(2,'".$row['user_name']."','{$row['password']}','{$row['email']}','{$row['web_id']}',{$row['user_id']},7);<br>";	
}*/
