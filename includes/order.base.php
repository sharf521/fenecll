...
class BaseOrder extends Object
{
     ...
     *    @return    array
     */
    function _get_shipping_methods($store_id,$cangkuid=0)
    {
        if (!$store_id)
        {
            return array();
		...
		return $shipping_model->find('store_id=' . $cangkuid);
    }
	...
	foreach($goods_info['items'] as $key=>$good)
	{
		$zong_feemoney+=$good['fee_money'];
		$zong_feejifen+=$good['fee_jifen'];
	}
...
/*$member_row=$shipping_model->getRow("select vip from ".DB_PREFIX."member where user_id='$userid' limit 1");
	...
	else
	{
 		...
 		if($member_row['vip']==1)
 		{
	 		$fee=$shipping_fee*$canshu['jifenxianjin']*(1+$canshu['lv21']);
 		}
 		else
 		{
	 		$fee=$shipping_fee*$canshu['jifenxianjin']*(1+$canshu['lv31']);
 		}
