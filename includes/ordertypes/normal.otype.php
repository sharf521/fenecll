...
class NormalOrder extends BaseOrder
{
    ...
    /* 显示订单表单 */
    function get_order_form($store_id,$cangkuid)
    {
        $data = array();
        ...
        /* 配送方式 */
        $data['shipping_methods']   = $this->_get_shipping_methods($store_id,$cangkuid);
...
//print_r($data);
	...
	$city_id=$cityrow['city_id'];
		...
		$mem=$this->_city_mod->getrow("select vip from ".DB_PREFIX."member where user_id='$user_id'");
        $vip=$mem['vip'];
		...
		if($vip==1)
		{
		$jifen_fee=$consignee_info['shipping_fee']*$jifenxianjin*(1+$lv21);
		...
		else
		{
		 $base_info['order_amount']  =   $base_info['goods_amount'] + $consignee_info['shipping_fee'] - $base_info['discount']-$base_info['youhuidiscount'];
		  $base_info['order_jifen']  =   $base_info['goods_jifen'] + $consignee_info['fee'] - $base_info['discount_jifen']-$base_info['youhuidiscount_jifen'];
		  $base_info['fanhuan_jia']  =   $base_info['order_amount']*$jifenxianjin;
		}
        $order_model =& m('order');
        ...
        foreach ($goods_info['items'] as $key => $value)
        {
			...
			{
				$jifen=$value['price']*$jifenxianjin*(1+$lv31);
			}
