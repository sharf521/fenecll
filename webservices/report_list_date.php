...
?>
	...
	{	
		...
		echoTh(array('时间','总积分','结算积分','信用(元)','状态','操作'));	
		...
		{			
			...
			//$startdate=$row['date'];
			//$enddate=date("Y-m-d",strtotime("+1days",strtotime($row['date'])));
			$row_1=$db->get_one("select sum(Mony) as xinyong from {process} where substring(IncomeTime,1,10)='{$row['date']}' and PlateNum>1");
			$xinyong=$row_1['xinyong'];
