...
class Member_model extends CI_Model
{
	...
	function replacelist_p2p($list, $rp)
    {
        $users = $this->get_userstr_noreplace($list);
        if ($users != '')
        {
            $query = $this->p2p->query("select user_id,username user_name,ws_user_id web_id from  {$this->p2p->dbprefix}user where ws_user_id in($users) limit 0,$rp");
            foreach ($list as $i => $v)
            {
                foreach ($query->result_array() as $row)
                {
